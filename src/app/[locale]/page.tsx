...
export default async function HomePage() {
  ...
  let membersCount = { count: 0 };
  let votesCount = { count: 0 };
  let factionsCount = { count: 0 };
  try {
    [[membersCount], [votesCount], [factionsCount]] = await Promise.all([
      db
      ...
      db
        ...
        .from(factions)
        .where(
          sql`${factions.id} in (select faction_id from members where is_current = true and faction_id is not null)`,
        ),
    ]);
  } catch (e) {
  ...
  const quickLinks = [
    {
      ...
      color: 'text-chart-1',
      bg: 'bg-chart-1/10',
      desc: 'quickLinks.membersDesc' as const,
    },
    {
      key: 'legislation' as const,
      href: '/legislation',
      icon: Gavel,
      color: 'text-chart-3',
      bg: 'bg-chart-3/10',
      desc: 'quickLinks.legislationDesc' as const,
    },
    {
      ...
      color: 'text-chart-4',
      bg: 'bg-chart-4/10',
      desc: 'quickLinks.factionsDesc' as const,
    },
  ];
      ...
      <section className="border-border/20 relative overflow-hidden border-b">
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-20 sm:px-6 sm:py-28 lg:py-32">
          <div className="flex flex-col items-center text-center">
            <AnimatedSection>
              <div className="bg-primary/10 ring-primary/20 glow-primary mb-8 flex h-18 w-18 items-center justify-center rounded-2xl ring-1">
                <Landmark className="text-primary h-9 w-9" />
              </div>
            </AnimatedSection>
          ...
          {stats.map(({ label, count, icon: Icon, color, bg }) => (
            ...
            >
              <CardContent className="flex items-center gap-4 p-6">
                <div
                  className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl ${bg} transition-transform group-hover:scale-110`}
                >
            ...
            <Link key={key} href={href}>
              <Card className="group glass-card hover-lift h-full overflow-hidden">
                <CardContent className="flex flex-col items-center gap-4 p-8 text-center">
                  <div
                    className={`flex h-14 w-14 items-center justify-center rounded-2xl ${bg} transition-all group-hover:scale-110 group-hover:shadow-md`}
                  >
                    <Icon className={`h-7 w-7 ${color}`} />
                  </div>
                  <div>
