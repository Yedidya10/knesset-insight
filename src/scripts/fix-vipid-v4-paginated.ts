 ...
 */
async function fetchAllV4ForMember(mkId: number, maxRetries = 5): Promise<V4Result[]> {
  const allResults: V4Result[] = [];
  const baseQs = `$filter=MkId eq ${mkId}&$select=VoteID,MkId,ResultCode&$orderby=VoteID desc&$format=json`;
  let url: string | null = `https://knesset.gov.il/OdataV4/ParliamentInfo/KNS_PlenumVoteResult?${baseQs}`;
...
async function main() {
  ...
  const k25VoteIds = new Set(
    (await db.select({ id: votes.id }).from(votes).where(eq(votes.knessetNum, 25)))
      .map(v => v.id)
  );
  ...
  for (const { memberId, name, newVipId } of CORRECTIONS) {
    ...
    console.log(`  ✓ v4: ${inserted} member_votes inserted (${v4Results.length - v4Rows.length} skipped - no matching vote)`);
  ...
  for (const { memberId, name } of CORRECTIONS) {
    ...
    `);
    const r = result[0];
    console.log(`  ${name}: ${r.total_votes} total (${r.legacy_votes} legacy + ${r.k25_votes} K25)`);
  }
