...
async function main() {
  ...
  for (const r of data.value) {
    console.log(`${String(r.MkId).padEnd(7)} | ${(r.FirstName + ' ' + r.LastName).padEnd(25)} | ${r.ResultCode}`);
  }

  // 2. Check Dan Illouz result
  console.log('\n=== Dan Illouz check ===');
  const illouz = data.value.find((r: any) => 
    r.LastName?.includes('אילוז') || r.LastName?.includes('Illouz') || r.MkId === 34520
  );
  console.log(`Illouz in vote 40000: ${illouz ? JSON.stringify(illouz) : 'NOT FOUND'}`);
  ...
  for (const r of data2.value || []) {
    console.log(`  VoteID=${r.VoteID}, ${r.FirstName} ${r.LastName}, ResultCode=${r.ResultCode}`);
  }
  ...
  for (const p of data3.value || []) {
    console.log(`  PersonID=${p.PersonID}, ${p.FirstName} ${p.LastName}, IsCurrent=${p.IsCurrent}`);
  }
  ...
  for (const entity of ['KNS_MkIndividual', 'kns_MKIndividual', 'KNS_MK']) {
    try {
      ...
      if (res.ok) {
        ...
        if (d.value?.[0]) console.log(`  Sample record keys: ${Object.keys(d.value[0]).join(', ')}`);
        if (d.value?.[0]) console.log(`  Sample: ${JSON.stringify(d.value[0]).substring(0, 300)}`);
      } else {
  ...
  for (const [name, id] of [['Buskila', 30799], ['Israel Katz', 46]] as const) {
    try {
      ...
      const res = await fetch(url, { signal: AbortSignal.timeout(10_000) });
      const d = await res.json();
      console.log(`${name} (PersonID=${id}): ${d['@odata.count'] ?? d.value?.length} results`);
    } catch (err: any) {
  ...
  try {
    ...
    const res = await fetch(url, { signal: AbortSignal.timeout(10_000) });
    const d = await res.json();
    console.log(`Search by LastName contains 'בוסקילה': ${d.value?.length} results`);
    for (const r of d.value || []) {
      console.log(`  MkId=${r.MkId}, VoteID=${r.VoteID}, ${r.FirstName} ${r.LastName}`);
    }
  } catch (err: any) {
