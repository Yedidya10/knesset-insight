...
async function main() {
  ...
  console.log('\n=== KNS_Person sample ===');
  try {
    const url = 'https://knesset.gov.il/OdataV4/ParliamentInfo/KNS_Person?$top=3&$format=json';
    const res = await fetch(url, { signal: AbortSignal.timeout(10_000) });
  ...
  const sampleVoteIds = [
    37786, 37800, 38000, 38500, 38793, 39000, 39200, 39500, 39660,
    40000, 40500, 41000, 41500, 42000, 42500, 43500, 44000, 44500,
    45000, 45200, 45500, 45700, 45800
  ];
    ...
    try {
      ...
      for (const r of d.value || []) {
        if (!allMkIdMap.has(r.MkId)) {
          allMkIdMap.set(r.MkId, { firstName: r.FirstName, lastName: r.LastName });
        }
      }
      process.stdout.write(`  Vote ${voteId}: ${d.value?.length || 0} results (total unique MKs: ${allMkIdMap.size})\n`);
    } catch (err: any) {
  ...
  let missingVipId = 0;
  let notInDb = 0;
  const corrections: { memberId: number; oldVipId: number | null; newVipId: number; name: string }[] = [];
  ...
  for (const [mkId, { firstName, lastName }] of allMkIdMap) {
    ...
    if (bestMatch.vipId === mkId) {
      ...
      missingVipId++;
      corrections.push({ memberId: bestMatch.id, oldVipId: null, newVipId: mkId, name });
    } else {
      wrongCount++;
      corrections.push({ memberId: bestMatch.id, oldVipId: bestMatch.vipId, newVipId: mkId, name });
    }
  }
  ...
  if (corrections.length > 0) {
    ...
    for (const c of corrections) {
      console.log(`  id=${c.memberId}: "${c.name}" ${c.oldVipId} → ${c.newVipId}`);
    }
  }
  ...
  for (const c of currentMissing) {
    console.log(`  id=${c.memberId}: "${c.name}" old=${c.oldVipId} → new=${c.newVipId}`);
  }
