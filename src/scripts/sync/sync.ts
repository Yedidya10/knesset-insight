...
async function main() {
  ...
  const { syncCommittees } =
    await import('../../pipeline/jobs/sync-committees');
  ...
  const { syncGovernments } =
    await import('../../pipeline/jobs/sync-governments');
  ...
  const { syncCommitteeSessions } =
    await import('../../pipeline/jobs/sync-committee-sessions');
  ...
  if (target === 'committeeSessions' || target === 'all') {
