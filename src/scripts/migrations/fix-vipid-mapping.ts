...
async function fetchLegacyVotesForMember(
  vipId: number,
  ...
  while (true) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        page = await fetchOData<ODataMemberVote>(
          'Votes',
...
async function main() {
  ...
  for (const correction of CORRECTIONS) {
    ...
    const deleted = await db
      .delete(memberVotes)
      .where(eq(memberVotes.memberId, memberId));
    ...
    try {
      ...
      const v4Results: { VoteID: number; MkId: number; ResultCode: number }[] =
        data.value || [];
  ...
  const affectedVoteIds = await db
    .select({ voteId: memberVotes.voteId })
    .from(memberVotes)
    .where(
      inArray(
        memberVotes.memberId,
        CORRECTIONS.map((c) => c.memberId),
      ),
    )
