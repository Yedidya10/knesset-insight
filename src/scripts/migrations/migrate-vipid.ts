...
async function main() {
  ...
  try {
    await sql.unsafe(
      'ALTER TABLE members ADD COLUMN IF NOT EXISTS vip_id integer',
    );
    console.log('Migration done: vip_id column added to members table');

    // Verify Dan Illouz (PersonID=30839) got vipId=34520
    const rows = await sql.unsafe(
      `SELECT id, knesset_id, vip_id, first_name, last_name FROM members WHERE knesset_id = 30839`,
    );
    console.log('Dan Illouz:', rows[0]);

    // Count members with vipId populated
    const counts = await sql.unsafe(
      `SELECT count(*) as total, count(vip_id) as with_vip_id FROM members`,
    );
