...
function FactionBarList({ entries, labels }: { entries: [string, FactionCounts][]; labels: { for: string; against: string; abstain: string } }) {
  ...
  return (
    <div className="space-y-1.5">
      {sorted.map(([partyName, counts]) => {
        ...
        return (
          <div key={partyName} className="rounded-lg bg-muted/30 px-3 py-2 transition-colors hover:bg-muted/50">
...
export default function FactionBreakdown({ coalitionFactions, oppositionFactions }: FactionBreakdownProps) {
  const t = useTranslations('votes');
  const labels = { for: t('for'), against: t('against'), abstain: t('abstain') };
  ...
  return (
    <>
      ...
      <div className="hidden md:grid md:grid-cols-2 md:gap-6">
        {hasCoalition && (
          <div>
            <h3 className="mb-2 text-sm font-semibold text-muted-foreground">{t('coalitionBreakdown')}</h3>
            <FactionBarList entries={coalitionFactions} labels={labels} />
          </div>
        )}
        {hasOpposition && (
          <div>
            <h3 className="mb-2 text-sm font-semibold text-muted-foreground">{t('oppositionBreakdown')}</h3>
        ...
        {hasCoalition && hasOpposition ? (
          <Tabs defaultValue="coalition">
            <TabsList className="w-full">
              <TabsTrigger value="coalition">{t('coalitionBreakdown')}</TabsTrigger>
              <TabsTrigger value="opposition">{t('oppositionBreakdown')}</TabsTrigger>
            </TabsList>
