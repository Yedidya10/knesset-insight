...
interface MemberPolicyStancesProps {
  memberId: number;
}

export default async function MemberPolicyStances({
  memberId,
}: MemberPolicyStancesProps) {
  ...
  // Get this member's votes on aligned votes
  const mvRows = await db
    ...
    .where(
      and(
        eq(memberVotes.memberId, memberId),
